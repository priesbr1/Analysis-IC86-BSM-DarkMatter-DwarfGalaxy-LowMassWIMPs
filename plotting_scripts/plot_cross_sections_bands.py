...
for channel in cross_sections.keys():
    ...
    for mass in cross_sections[channel].keys():
        m_WIMP.append(mass)
        sigma_v.append([])
        for sigma in cross_sections[channel][mass]["Cross Section"].keys():
            sigma_v[-1].append(cross_sections[channel][mass]["Cross Section"][sigma])
        sigma_v[-1] = sorted(sigma_v[-1])
    
    ax.plot(m_WIMP, [sigma_v[i][2] for i in range(len(sigma_v))], color=colors[channel], linestyle="-", label=legends[channel], linewidth=2)
