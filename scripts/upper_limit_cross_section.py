...
parser.add_argument("-t", "--unblinded_trials", type=str, default="/mnt/home/priesbr1/DM_Search/data/trials_results/trials_unblinded_360deg_Jmax/",
                    dest="unblinded_trials", help="folder containing results from unblinded trials")
...
parser.add_argument("-o", "--outfile", type=str, default="/mnt/home/priesbr1/DM_Search/data/cross_section_results/cross_section_results_upper_limits_360deg_Jmax_distributed_erange.npy",
...
unblinded_file = args.unblinded_trials + "unblinded_trials_" + args.channel + "_" + str(args.mass) + ".txt"
unblinded_trials = np.genfromtxt(unblinded_file, delimiter="\t")
TS_thresh = unblinded_trials[-2]
print("Unblinded best-fit TS:", TS_thresh)
...
for ni in sorted(np.unique(sig_ni)):
    ...
    TS_frac = len(np.where(ni_TS > TS_thresh)[0])/len(ni_TS)
...
ns_thresh = int(np.round(interpolator_ns(0.9)))
TS_frac_thresh = interpolator_frac(ns_thresh)
print("Estimated upper limit threshold: %i"%ns_thresh)
print("Estimated fraction of trials above upper limit threshold: %s"%(TS_frac_thresh))

flux = inj.mu2flux(ns_thresh)
